...
#define Restore_after_gc
  Alloc_small(res, Double_wosize, Double_tag, PROF_FLOAT_CP_DOUBLE);
#undef Setup_for_gc
...
CAMLprim value caml_frexp_float(value f)
{
  ...
  res = caml_alloc_tuple_loc(2, PROF_FLOAT_FREXP_FLOAT);
...
CAMLprim value caml_modf_float(value f)
{
  ...
  res = caml_alloc_tuple_loc(2, PROF_FLOAT_MODF_FLOAT);
  Field(res, 0) = quo;
  Field(res, 1) = rem;
  CAMLreturn (res);
}
