...
CAMLprim value caml_dynlink_open_lib(value mode, value filename)
{
  ...
  result = caml_alloc_small_loc(1, Abstract_tag, PROF_DYNLINK_OPEN_LIB);
...
CAMLprim value caml_dynlink_lookup_symbol(value handle, value symbolname)
{
  ...
  result = caml_alloc_small_loc(1, Abstract_tag, PROF_DYNLINK_LOOKUP_SYM);
...
CAMLprim value caml_dynlink_get_current_libs(value unit)
{
  ...
  res = caml_alloc_tuple_loc(shared_libs.size, PROF_TUPLE_DYNLINK_GET_CUR_LIBS);
  for (i = 0; i < shared_libs.size; i++) {
    value v = caml_alloc_small_loc(1, Abstract_tag, PROF_DYNLINK_GET_CUR_LIBS);
