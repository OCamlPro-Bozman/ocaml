...
#define Restore_after_gc
  Alloc_small(res, Double_wosize, Double_tag, PROF_FLOAT_GET_FLOAT);
#undef Setup_for_gc
...
#define Setup_for_gc
#define Restore_after_gc
  Alloc_small(res, Double_wosize, Double_tag, PROF_FLOAT_UNSAFE_GET_FLOAT);
#undef Setup_for_gc
#undef Restore_after_gc
  Store_double_val(res, d);
  return res;
...
CAMLprim value caml_make_vect(value len, value init)
{
  ...
  if (size == 0) {
    ...
    res = caml_alloc_loc(wsize, Double_array_tag, PROF_DOUBLE_ARRAYS_MK_VECT);
    for (i = 0; i < size; i++) {
    ...
    if (size < Max_young_wosize) {
      res = caml_alloc_small_loc(size, 0, PROF_ARRAYS_MK_VECT_1);
      for (i = 0; i < size; i++) Field(res, i) = init;
    }
    else if (Is_block(init) && Is_young(init)) {
      caml_minor_collection();
      res = caml_alloc_shr_loc(size, 0, PROF_ARRAYS_MK_VECT_2);
      for (i = 0; i < size; i++) Field(res, i) = init;
      res = caml_check_urgent_gc (res);
    }
    else {
      res = caml_alloc_shr_loc(size, 0, PROF_ARRAYS_MK_VECT_3);
...
CAMLprim value caml_make_array(value init)
{
  ...
  if (size == 0) {
    ...
    if (Is_long(v)
      ...
      wsize = size * Double_wosize;
      res = caml_alloc_small_loc(wsize, Double_array_tag, PROF_DOUBLE_ARRAYS_MK_ARRAY);
      for (i = 0; i < size; i++) {
                               ...
                               intnat lengths[/*num_arrays*/])
{
  ...
  if (size == 0) {
    ...
    res = caml_alloc_loc(wsize, Double_array_tag, PROF_DOUBLE_ARRAYS_GATHER);
    ...
    res = caml_alloc_small_loc(size, 0, PROF_ARRAYS_GATHER_1);
    for (i = 0, pos = 0; i < num_arrays; i++) {
    ...
    res = caml_alloc_shr_loc(size, 0, PROF_ARRAYS_GATHER_2);
    pos = 0;
    for (i = 0, pos = 0; i < num_arrays; i++) {
